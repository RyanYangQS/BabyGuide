# 养娃不易小程序 - 技术方案(微信小程序版)

## 一、技术栈选型

### 1.1 前端技术栈

#### 框架选择
- **框架**: Taro 3.x (React语法) 或 uni-app (Vue语法)
- **语言**: TypeScript
- **样式**: Sass/Less
- **状态管理**: 
  - Taro: Redux Toolkit / Zustand
  - uni-app: Pinia / Vuex

#### 为什么选择Taro?
- ✅ 支持React语法,学习成本低
- ✅ 一套代码,多端运行(微信/支付宝/字节跳动等)
- ✅ 完善的TypeScript支持
- ✅ 丰富的组件库(Taro UI / NutUI)
- ✅ 活跃的社区和完善的文档

#### 为什么选择uni-app?
- ✅ 支持Vue语法,开发体验好
- ✅ 生态丰富,插件市场完善
- ✅ 一套代码,多端运行
- ✅ 性能优化做得好
- ✅ 中文文档完善

**推荐选择**: **Taro 3.x + React + TypeScript**

### 1.2 后端技术栈

#### 方案一: Supabase (推荐)
- **数据库**: Supabase PostgreSQL
- **认证**: Supabase Auth
- **存储**: Supabase Storage
- **实时**: Supabase Realtime

#### 方案二: 微信云开发
- **数据库**: 云开发数据库(NoSQL)
- **认证**: 微信登录
- **存储**: 云存储
- **云函数**: Node.js

#### 方案三: 自建后端
- **框架**: NestJS / Express
- **数据库**: PostgreSQL / MongoDB
- **部署**: Vercel / Railway

**推荐选择**: **Supabase** (免费额度充足,功能完善)

### 1.3 部署方案

#### 小程序部署
- **平台**: 微信小程序平台
- **审核**: 微信小程序审核
- **发布**: 微信小程序发布

#### 后端部署
- **平台**: Vercel (如果使用Next.js API Routes)
- **或**: Supabase (无需额外部署)

---

## 二、项目结构

### 2.1 Taro项目结构

```
baby-guide-miniprogram/
├── src/
│   ├── pages/                  # 页面
│   │   ├── index/             # 首页
│   │   │   ├── index.tsx
│   │   │   ├── index.scss
│   │   │   └── index.config.ts
│   │   ├── temperature/       # 体温记录
│   │   ├── medicine/          # 用药管理
│   │   ├── profile/           # 我的
│   │   └── login/             # 登录
│   ├── components/            # 组件
│   │   ├── TemperatureChart/  # 体温图表
│   │   ├── MedicineCard/      # 用药卡片
│   │   └── Common/            # 通用组件
│   ├── services/              # API服务
│   │   ├── api.ts            # API封装
│   │   ├── supabase.ts       # Supabase客户端
│   │   └── wechat.ts         # 微信API
│   ├── store/                 # 状态管理
│   │   ├── index.ts
│   │   └── modules/
│   ├── utils/                 # 工具函数
│   │   ├── request.ts        # 请求封装
│   │   ├── storage.ts        # 本地存储
│   │   └── helpers.ts        # 辅助函数
│   ├── constants/             # 常量
│   │   └── index.ts
│   ├── types/                 # TypeScript类型
│   │   └── index.ts
│   ├── app.tsx               # 入口文件
│   ├── app.scss              # 全局样式
│   └── app.config.ts         # 全局配置
├── config/                    # Taro配置
│   ├── index.ts
│   ├── dev.ts
│   └── prod.ts
├── package.json
└── tsconfig.json
```

---

## 三、核心功能实现

### 3.1 项目初始化

```bash
# 安装Taro CLI
npm install -g @tarojs/cli

# 创建项目
taro init baby-guide-miniprogram

# 选择模板
# 框架: React
# 语言: TypeScript
# CSS预处理器: Sass
```

### 3.2 Supabase集成

```typescript
// src/services/supabase.ts
import { createClient } from '@supabase/supabase-js'

const supabaseUrl = process.env.TARO_APP_SUPABASE_URL
const supabaseAnonKey = process.env.TARO_APP_SUPABASE_ANON_KEY

export const supabase = createClient(supabaseUrl, supabaseAnonKey)

// 微信登录
export async function loginWithWechat() {
  // 获取微信登录code
  const { code } = await Taro.login()
  
  // 发送到Supabase
  const { data, error } = await supabase.auth.signInWithOAuth({
    provider: 'wechat',
    options: {
      queryParams: {
        code
      }
    }
  })
  
  return { data, error }
}

// 手机号登录
export async function loginWithPhone(phone: string, code: string) {
  const { data, error } = await supabase.auth.verifyOtp({
    phone,
    token: code,
    type: 'sms'
  })
  
  return { data, error }
}
```

### 3.3 体温记录功能

```typescript
// src/services/api/temperature.ts
import { supabase } from '../supabase'
import Taro from '@tarojs/taro'

// 添加体温记录
export async function addTemperatureRecord(data: {
  childId: string
  temperature: number
  measureTime: string
  measurePart: string
  notes?: string
}) {
  try {
    const { data: record, error } = await supabase
      .from('temperature_records')
      .insert({
        child_id: data.childId,
        temperature: data.temperature,
        measure_time: data.measureTime,
        measure_part: data.measurePart,
        notes: data.notes
      })
      .select()
      .single()

    if (error) throw error
    
    Taro.showToast({
      title: '记录成功',
      icon: 'success'
    })
    
    return { record }
  } catch (error) {
    Taro.showToast({
      title: '记录失败',
      icon: 'error'
    })
    return { error }
  }
}

// 获取体温记录
export async function getTemperatureRecords(
  childId: string,
  days: number = 7
) {
  const startDate = new Date()
  startDate.setDate(startDate.getDate() - days)
  
  const { data, error } = await supabase
    .from('temperature_records')
    .select('*')
    .eq('child_id', childId)
    .gte('measure_time', startDate.toISOString())
    .order('measure_time', { ascending: false })

  return { data, error }
}
```

### 3.4 智能用药管理

```typescript
// src/services/api/medicine.ts
import { supabase } from '../supabase'
import Taro from '@tarojs/taro'

// 检查用药间隔
export async function checkMedicineInterval(
  childId: string,
  medicineName: string
) {
  // 获取药品信息
  const { data: medicine } = await supabase
    .from('medicines')
    .select('*')
    .eq('name', medicineName)
    .single()

  if (!medicine) {
    return { canTake: false, reason: '药品信息不存在' }
  }

  // 获取最近24小时的用药记录
  const twentyFourHoursAgo = new Date(Date.now() - 24 * 60 * 60 * 1000)
  const { data: recentRecords } = await supabase
    .from('medicine_records')
    .select('*')
    .eq('child_id', childId)
    .eq('medicine_name', medicineName)
    .gte('take_time', twentyFourHoursAgo.toISOString())
    .order('take_time', { ascending: false })

  // 检查24小时用药次数
  if (recentRecords && recentRecords.length >= medicine.max_daily_count) {
    return {
      canTake: false,
      reason: `24小时内已用药${recentRecords.length}次,已达上限`
    }
  }

  // 检查用药间隔
  if (recentRecords && recentRecords.length > 0) {
    const lastRecord = recentRecords[0]
    const lastTakeTime = new Date(lastRecord.take_time)
    const nextAvailableTime = new Date(
      lastTakeTime.getTime() + medicine.min_interval_hours * 60 * 60 * 1000
    )

    if (new Date() < nextAvailableTime) {
      return {
        canTake: false,
        reason: `距离下次用药还需等待`,
        nextAvailableTime
      }
    }
  }

  return { canTake: true }
}

// 添加用药记录
export async function addMedicineRecord(data: {
  childId: string
  medicineName: string
  dosage: string
  takeTime: string
  notes?: string
}) {
  // 先检查用药间隔
  const checkResult = await checkMedicineInterval(
    data.childId,
    data.medicineName
  )

  if (!checkResult.canTake) {
    Taro.showModal({
      title: '用药提醒',
      content: checkResult.reason,
      showCancel: false
    })
    return { error: checkResult.reason }
  }

  // 计算下次可用时间
  const { data: medicine } = await supabase
    .from('medicines')
    .select('min_interval_hours')
    .eq('name', data.medicineName)
    .single()

  const nextAvailableTime = new Date(
    new Date(data.takeTime).getTime() + medicine.min_interval_hours * 60 * 60 * 1000
  )

  // 插入记录
  const { data: record, error } = await supabase
    .from('medicine_records')
    .insert({
      child_id: data.childId,
      medicine_name: data.medicineName,
      dosage: data.dosage,
      take_time: data.takeTime,
      next_available_time: nextAvailableTime.toISOString(),
      notes: data.notes
    })
    .select()
    .single()

  if (error) {
    Taro.showToast({
      title: '记录失败',
      icon: 'error'
    })
    return { error }
  }

  Taro.showToast({
    title: '记录成功',
    icon: 'success'
  })

  return { record }
}
```

### 3.5 体温图表组件

```typescript
// src/components/TemperatureChart/index.tsx
import { View } from '@tarojs/components'
import { LineChart } from 'react-native-chart-kit'
import { Dimensions } from 'react-native'
import './index.scss'

interface TemperatureChartProps {
  data: Array<{
    time: string
    temperature: number
  }>
}

export default function TemperatureChart({ data }: TemperatureChartProps) {
  const chartData = {
    labels: data.map(d => d.time),
    datasets: [{
      data: data.map(d => d.temperature),
      color: (opacity = 1) => `rgba(74, 144, 226, ${opacity})`,
      strokeWidth: 2
    }]
  }

  const chartConfig = {
    backgroundGradientFrom: '#FFFFFF',
    backgroundGradientTo: '#FFFFFF',
    color: (opacity = 1) => `rgba(0, 0, 0, ${opacity})`,
    strokeWidth: 2,
    decimalPlaces: 1,
    propsForDots: {
      r: '4',
      strokeWidth: '2',
      stroke: '#4A90E2'
    }
  }

  return (
    <View className='temperature-chart'>
      <LineChart
        data={chartData}
        width={Dimensions.get('window').width - 32}
        height={200}
        chartConfig={chartConfig}
        bezier
      />
    </View>
  )
}
```

### 3.6 页面示例

```typescript
// src/pages/index/index.tsx
import { View, Text } from '@tarojs/components'
import { useEffect, useState } from 'react'
import Taro, { useDidShow } from '@tarojs/taro'
import { getTemperatureRecords } from '../../services/api/temperature'
import { getMedicineRecords } from '../../services/api/medicine'
import TemperatureChart from '../../components/TemperatureChart'
import './index.scss'

export default function Index() {
  const [temperatureRecords, setTemperatureRecords] = useState([])
  const [medicineRecords, setMedicineRecords] = useState([])
  const [currentChild, setCurrentChild] = useState(null)

  useDidShow(() => {
    loadData()
  })

  const loadData = async () => {
    // 获取当前儿童
    const child = Taro.getStorageSync('currentChild')
    if (!child) {
      Taro.navigateTo({ url: '/pages/profile/index' })
      return
    }
    
    setCurrentChild(child)
    
    // 加载数据
    const { data: tempData } = await getTemperatureRecords(child.id)
    const { data: medData } = await getMedicineRecords(child.id)
    
    setTemperatureRecords(tempData || [])
    setMedicineRecords(medData || [])
  }

  return (
    <View className='index'>
      {/* 儿童档案卡片 */}
      <View className='child-card'>
        <View className='child-info'>
          <View className='child-avatar'>👶</View>
          <View className='child-details'>
            <Text className='child-name'>{currentChild?.name}</Text>
            <Text className='child-age'>{currentChild?.age}</Text>
          </View>
        </View>
      </View>

      {/* 今日概览 */}
      <View className='overview-section'>
        <View className='section-title'>今日概览</View>
        <View className='overview-cards'>
          <View className='overview-card'>
            <Text className='value'>{temperatureRecords[0]?.temperature || '--'}℃</Text>
            <Text className='label'>体温</Text>
          </View>
          <View className='overview-card'>
            <Text className='value'>{medicineRecords.length}次</Text>
            <Text className='label'>用药</Text>
          </View>
        </View>
      </View>

      {/* 体温图表 */}
      {temperatureRecords.length > 0 && (
        <View className='chart-section'>
          <View className='section-title'>体温趋势</View>
          <TemperatureChart data={temperatureRecords} />
        </View>
      )}

      {/* 快速记录按钮 */}
      <View className='quick-actions'>
        <View 
          className='action-btn'
          onClick={() => Taro.navigateTo({ url: '/pages/temperature/add' })}
        >
          <Text className='icon'>🌡️</Text>
          <Text className='text'>体温</Text>
        </View>
        <View 
          className='action-btn'
          onClick={() => Taro.navigateTo({ url: '/pages/medicine/add' })}
        >
          <Text className='icon'>💊</Text>
          <Text className='text'>用药</Text>
        </View>
      </View>
    </View>
  )
}
```

---

## 四、小程序配置

### 4.1 app.config.ts

```typescript
export default defineAppConfig({
  pages: [
    'pages/index/index',
    'pages/temperature/index',
    'pages/temperature/add',
    'pages/medicine/index',
    'pages/medicine/add',
    'pages/profile/index',
    'pages/login/index'
  ],
  window: {
    backgroundTextStyle: 'light',
    navigationBarBackgroundColor: '#fff',
    navigationBarTitleText: '养娃不易',
    navigationBarTextStyle: 'black'
  },
  tabBar: {
    color: '#999',
    selectedColor: '#4A90E2',
    backgroundColor: '#fff',
    list: [
      {
        pagePath: 'pages/index/index',
        text: '首页',
        iconPath: 'assets/icons/home.png',
        selectedIconPath: 'assets/icons/home-active.png'
      },
      {
        pagePath: 'pages/temperature/index',
        text: '体温',
        iconPath: 'assets/icons/temperature.png',
        selectedIconPath: 'assets/icons/temperature-active.png'
      },
      {
        pagePath: 'pages/medicine/index',
        text: '用药',
        iconPath: 'assets/icons/medicine.png',
        selectedIconPath: 'assets/icons/medicine-active.png'
      },
      {
        pagePath: 'pages/profile/index',
        text: '我的',
        iconPath: 'assets/icons/profile.png',
        selectedIconPath: 'assets/icons/profile-active.png'
      }
    ]
  }
})
```

### 4.2 project.config.json

```json
{
  "miniprogramRoot": "dist/",
  "projectname": "baby-guide",
  "description": "养娃不易小程序",
  "appid": "your-app-id",
  "setting": {
    "urlCheck": false,
    "es6": false,
    "enhance": false,
    "postcss": false,
    "preloadBackgroundData": false,
    "minified": false,
    "newFeature": false,
    "coverView": true,
    "nodeModules": false,
    "autoAudits": false,
    "showShadowRootInWxmlPanel": true,
    "scopeDataCheck": false,
    "uglifyFileName": false,
    "checkInvalidKey": true,
    "checkSiteMap": true,
    "uploadWithSourceMap": true,
    "compileHotReLoad": false,
    "lazyloadPlaceholderEnable": false,
    "useMultiFrameRuntime": true,
    "useApiHook": true,
    "useApiHostProcess": true,
    "babelSetting": {
      "ignore": [],
      "disablePlugins": [],
      "outputPath": ""
    },
    "useIsolateContext": true,
    "userConfirmedBundleSwitch": false,
    "packNpmManually": false,
    "packNpmRelationList": [],
    "minifyWXSS": true,
    "disableUseStrict": false,
    "showES6CompileOption": false,
    "useCompilerPlugins": [
      "typescript"
    ]
  },
  "compileType": "miniprogram",
  "libVersion": "2.25.0",
  "condition": {}
}
```

---

## 五、开发流程

### 5.1 本地开发

```bash
# 安装依赖
npm install

# 微信小程序开发模式
npm run dev:weapp

# 打开微信开发者工具
# 导入项目根目录
# AppID使用测试号或正式AppID
```

### 5.2 构建发布

```bash
# 构建微信小程序
npm run build:weapp

# 在微信开发者工具中上传代码
# 提交审核
# 发布上线
```

---

## 六、成本估算

### 6.1 开发成本

| 项目 | 成本 | 说明 |
|------|------|------|
| 小程序注册 | 0元 | 个人开发者免费 |
| Supabase | 0元 | 500MB数据库免费 |
| 域名 | 已有 | yangqingsong.top |
| SSL证书 | 0元 | 小程序自带 |
| **总成本** | **0元** | 完全免费 |

### 6.2 维护成本

- **服务器**: 0元 (Supabase免费额度)
- **存储**: 0元 (1GB内免费)
- **带宽**: 0元 (小程序自带CDN)
- **总成本**: 0元/月

---

## 七、技术难点

### 7.1 微信登录

**问题**: Supabase的微信OAuth需要企业认证

**解决方案**:
1. 使用手机号登录(推荐)
2. 自建认证服务
3. 使用微信云开发

### 7.2 图表展示

**问题**: 小程序中图表库较少

**解决方案**:
1. 使用 `react-native-chart-kit` (Taro兼容)
2. 使用 `echarts-for-weixin`
3. Canvas自绘

### 7.3 实时订阅

**问题**: 小程序不支持WebSocket长连接

**解决方案**:
1. 轮询方式获取数据
2. 使用微信云开发的实时数据推送
3. 小程序后台运行时断开连接

---

## 八、后续扩展

### 8.1 多端适配

Taro支持一套代码多端运行:

```bash
# 微信小程序
npm run dev:weapp

# 支付宝小程序
npm run dev:alipay

# 字节跳动小程序
npm run dev:tt

# H5
npm run dev:h5

# React Native
npm run dev:rn
```

### 8.2 Web版本

如果需要Web版本,可以:

1. **使用Taro H5**
   ```bash
   npm run build:h5
   ```
   部署到 `baby.yangqingsong.top`

2. **使用Next.js重写**
   - 更好的SEO
   - SSR支持
   - 更丰富的Web功能

---

## 九、总结

### 9.1 技术方案对比

| 方案 | 优点 | 缺点 | 推荐度 |
|------|------|------|--------|
| **Taro + Supabase** | 多端支持、免费、TypeScript | 需要学习Taro | ⭐⭐⭐⭐⭐ |
| uni-app + Supabase | Vue生态、插件丰富 | 性能稍差 | ⭐⭐⭐⭐ |
| 微信云开发 | 原生集成、无需后端 | 绑定微信生态 | ⭐⭐⭐⭐ |
| 原生小程序 | 性能最好 | 只能微信使用 | ⭐⭐⭐ |

### 9.2 最终推荐

**技术栈**: Taro 3.x + React + TypeScript + Supabase

**核心优势**:
- ✅ 完全免费
- ✅ 多端支持
- ✅ 开发效率高
- ✅ 代码可维护性强
- ✅ 可扩展到Web版本

**开发周期**: 4-6周

**总成本**: 0元

---

## 十、快速开始

### 10.1 创建项目

```bash
# 1. 安装Taro CLI
npm install -g @tarojs/cli

# 2. 创建项目
taro init baby-guide-miniprogram

# 3. 选择配置
# ✔ 请输入项目介绍: 养娃不易小程序
# ✔ 请选择框架: React
# ✔ 请选择 CSS 预处理器: Sass
# ✔ 请选择模板: 默认模板

# 4. 进入项目
cd baby-guide-miniprogram

# 5. 安装依赖
npm install @supabase/supabase-js
npm install react-native-chart-kit
npm install react-native-svg

# 6. 启动开发
npm run dev:weapp
```

### 10.2 配置Supabase

```bash
# 创建 .env 文件
TARO_APP_SUPABASE_URL=your_supabase_url
TARO_APP_SUPABASE_ANON_KEY=your_supabase_anon_key
```

### 10.3 开始开发

1. 在微信开发者工具中导入项目
2. 使用测试AppID或正式AppID
3. 开始开发功能

---

这个方案更适合微信小程序开发,完全免费,且支持多端扩展! 🎊
